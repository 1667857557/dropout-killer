routing_fixture <- function() {
  set.seed(901)
  x <- matrix(rpois(24 * 24, 2), 24, 24,
              dimnames = list(paste0("g", 1:24), paste0("c", 1:24)))
  z <- matrix(rnorm(24 * 3), 24, 3,
              dimnames = list(colnames(x), paste0("PC_", 1:3)))
  list(x = x, z = z, group = rep(c("B", "T"), each = 12))
}

routing_test_model <- function() {
  set.seed(902)
...
test_that("RNA opt-out ignores ambiguous ATAC assays while WNN requires disambiguation", {
  ...
  obj$broad <- d$group
  rna <- suppressWarnings(dropout_killer_seurat(
    obj, modality = "rna", atac_assay = "unused", group_by = "broad",
    lean_model = routing_test_model(), rank = 3, gamma = 4,
...
test_that("automatic RNA calibration and inference use the same geometry pipeline", {
  d <- routing_fixture(); d$x[1:4, 1:12] <- 0; d$x[5:8, 13:24] <- 0
  run <- function(z, model = NULL) dropout_killer(
    d$x, z, group = d$group, rank = 3, gamma = 4,
    k_knn = 4, neighbor_k = 5, seed = 42, lean_model = model,
    lean_control = list(seeds = c(31, 32)), recovery_method = "neighbor"
  )
  first <- run(d$z)
  other <- d$z[rev(seq_len(nrow(d$z))), , drop = FALSE]
  rownames(other) <- rownames(d$z)
  second <- run(other)
  expected <- .dk_lean_rna_geometry(
    .dk_lean_svd(.dk_alra_library_log(d$x), 3, 42)$embedding,
    d$group, 4, 4
  )
  ...
  for (s in names(first$membership_fit$graphs))
    expect_equal(
      igraph::as_adjacency_matrix(first$membership_fit$graphs[[s]]),
  ...
  expect_error(
    dropout_killer(d$x, d$z, group = d$group, lean_geometry = expected),
    "custom geometry requires a fitted lean_model"
  )
  expect_error(
    dropout_killer(
      d$x, d$z, group = d$group,
