...
validate_dropout_result <- function(result, original, tolerance = 0) {
  ...
  if (inherits(original, "sparseMatrix")) {
    sm <- summary(original); ov <- if (nrow(sm)) as.numeric(original[cbind(sm$i, sm$j)]) else numeric()
  ...
  if (inherits(result$expression, "sparseMatrix") || inherits(original, "sparseMatrix")) {
    delta <- result$expression - original; ds <- summary(delta)
  ...
  vals <- if (inherits(result$expression, "sparseMatrix")) {
    if ("x" %in% methods::slotNames(result$expression)) methods::slot(result$expression, "x") else rep.int(1, length(summary(result$expression)$i))
