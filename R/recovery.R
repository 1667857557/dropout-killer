...
.dk_recover_events <- function(x, events, membership, embedding = NULL,
  ...
  if (recovery_method == "p1_stabilized_state") {
    if (factor_target != "positive") stop("p1_stabilized_state implements the positive-conditional recovery target only", call. = FALSE)
    if (is.null(embedding)) stop("embedding is required for recovery_method='p1_stabilized_state'", call. = FALSE)
...
#'
#' The production engine is `p1_stabilized_state`: target-gene folds are removed
#' from predictor construction, the target-safe state is smoothed once over the
#' hierarchy/embedding geometry, and positive-donor ridge predictions use analytic
#' leave-one-out shrinkage and bias calibration. Comparison engines remain available
#' only through this lower-level expert API. Observed nonzero values are never
#' overwritten.
#'
#' @export
recover_dropout_expression <- function(x, mask, membership, embedding = NULL,
  ...
  recovery_method <- match.arg(recovery_method)
  factor_target <- match.arg(factor_target)
  z <- if (recovery_method %in% c("p1_stabilized_state", "neighbor", "tree_local_factor")) .dk_align_embedding(embedding, nm$cells) else NULL
